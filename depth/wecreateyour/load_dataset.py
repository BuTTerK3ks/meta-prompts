...
class ThreeDCDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            if self.resize_size:
                def resize_keep_aspect(image, target_size, fill_value=0):
                    ...
                    if len(image_resized.shape) == 2:  # Grayscale image
                        image_resized = np.stack([image_resized] * 3, axis=-1)  # Convert grayscale to RGB

                    # Handle RGBA images by removing the alpha channel
                    elif image_resized.shape[2] == 4:  # If RGBA, remove the alpha channel
                        image_resized = image_resized[:, :, :3]

                    # Create a new image with the target size and fill with the fill_value
            ...
            # Masks are typically long type
            # Extend the dimension at the front for the mask tensor
            mask_tensor = mask_tensor.unsqueeze(0)  # Add a channel dimension to the mask tensor
